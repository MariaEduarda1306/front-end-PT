...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function loadCertificateData(id) {
        try {
            ...
            document.getElementById('nome-atividade').value = cert.nome_certificado;
            document.getElementById('instituicao').value = cert.instituicao;
            document.getElementById('carga-horaria').value = cert.carga_horaria_solicitada;

            // Preenche a categoria e atualiza o dropdown customizado
            const catSelect = document.getElementById('categoria');
            catSelect.value = cert.categoria_id;
            
            const customWrapper = catSelect.nextElementSibling;
            const triggerSpan = customWrapper.querySelector('.custom-select-trigger span');
            customWrapper.querySelectorAll('.custom-option').forEach(opt => {
                if (opt.dataset.value == cert.categoria_id) {
            ...
            if (cert.data_emissao) {
                document.getElementById('data_emissao').value = cert.data_emissao;
                const [ano, mes, dia] = cert.data_emissao.split('-');
    ...
    form.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(result.message || 'Ocorreu um erro ao salvar o certificado.');
            }
    ...
    try {
        ...
        certificados.forEach(cert => {
            ...
            if (cert.status === 'ENTREGUE') {
                acoesHTML = `
                    <div class="actions-section" style="margin-top: 1.5rem; padding-top: 1rem; border-top: 1px solid var(--glass-border); display: flex; gap: 10px;">
                        <button class="btn btn-secondary btn-edit-cert" data-id="${cert.id}" style="display: flex; align-items: center; gap: 5px;">
                            <i class="fas fa-edit"></i> Editar Envio
                        </button>
                        <button class="btn btn-delete-cert" data-id="${cert.id}" style="background-color: var(--status-reprovado); color: white; border: none; padding: 0.5rem 1rem; border-radius: 4px; cursor: pointer; display: flex; align-items: center; gap: 5px;">
                ...
                <div class="accordion-item">
                    ...
                    <div class="accordion-content">
                        <div class="content-wrapper">
                            ...
                            ${acoesHTML} <div class="preview-section" style="margin-top: 1.5rem;">
    ...
    if (avatarWrapper) {
        ...
        if (removeBtn) {
            removeBtn.addEventListener('click', () => {
                loggedInUser.avatar_preview = null;
